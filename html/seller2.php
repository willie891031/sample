<?php 
	...
	while($row=$result->fetch_assoc())
	{
		...
		if(isset($_POST[$num.Y])){
			$ylike=$row['ylike']+1;
			$sql2="UPDATE `sell_problem` SET `ylike`=".$ylike." where `num`='".$num."'";
			$result=mysqli_query($db_link,$sql2); 
			echo"<script>alert('謝謝您寶貴的意見');location.href='seller.php'</script>";
		}
		if(isset($_POST[$num.N])){
			$nlike=$row['nlike']+1;
			$sql2="UPDATE `sell_problem` SET `nlike`=".$nlike." where `num`='".$num."'";
			$result=mysqli_query($db_link,$sql2); 
